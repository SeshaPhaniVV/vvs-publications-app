            ...
            <thead class="vtl-thead">
              <tr class="vtl-thead-tr">
                <th v-if="hasCheckbox" class="vtl-thead-th vtl-checkbox-th">
                  <div>
                    <input v-model="setting.isCheckAll" type="checkbox" class="vtl-thead-checkbox" />
                  </div>
                </th>
                <th
                  ...
                  <tr v-if="groupingKey != ''" class="vtl-tbody-tr vtl-group-tr">
                    <td :colspan="hasCheckbox ? columns.length + 1 : columns.length" class="vtl-tbody-td vtl-group-td">
                      <div class="flex">
                        ...
                        <div
                          class="ml-2"
                          v-html="groupingDisplay ? groupingDisplay(groupingIndex) : groupingIndex"
                        ></div>
                      </div>
                    </td>
                  </tr>
      ...
      <div v-if="rows.length > 0" class="vtl-paging vtl-row">
        <template v-if="!setting.isHidePaging">
          ...
          <div class="vtl-paging-change-div col-sm-12 col-md-4">
            ...
            <select v-model="setting.pageSize" class="vtl-paging-count-dropdown">
              <option v-for="pageOption in pageOptions" :key="pageOption.value" :value="pageOption.value">
                {{ pageOption.text }}
              </option>
            </select>
...
export default defineComponent({
  ...
  emits: ['return-checked-rows', 'do-search', 'is-finished', 'get-now-page', 'row-clicked', 'row-toggled'],
  setup(props, { emit, slots }) {
    const localTable = ref(null);

    // 檢查下拉選單中是否包含預設一頁顯示筆數 (Validate dropdown's values have page-size value or not)
    const defaultPageSize = props.pageOptions.length > 0 ? ref(props.pageOptions[0].value) : ref(props.pageSize);
        ...
        if (props.startCollapsed || props.isKeepCollapsed) {
          for (const [groupIndex, el] of Object.entries(toggleButtonRefs.value)) {
            if (el) {
              ...
              if (props.isKeepCollapsed && groupingToggleStatus.value[groupIndex] !== undefined) {
    ...
    const toggleGroup = (groupIndex) => {
      const targetEl = toggleButtonRefs.value[groupIndex];
      if (targetEl) {
